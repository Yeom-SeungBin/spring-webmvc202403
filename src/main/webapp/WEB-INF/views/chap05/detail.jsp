...
<div id="wrap" class="form-container">
    ...
    <script>
        ...
        function renderReplies(replyList) {
            ...
            // 객체 디스트럭처링 (댓글 수, 페이지메이커, 댓글 목록으로 분해)
        ...
        // 서버에 실시간으로 비동기통신해서 JSON을 받아오는 함수
        function fetchGetReplies(pageNum = 1) { // 댓글 목록 요청 시 페이지 번호 전달(전달 안되면 기본값 1)
            // fetch 함수를 통해 비동기통신 진행할 때 GET 요청은 요청에 관련한 객체를 따로 전달하지 않습니다.
        ...
        function makePageButtonClickHandler() {
            ...
            $pageUl.onclick = e => {
                ...
                // 이벤트 타겟이 a 태그가 아니면 href 속성을 못가져올 수 있으니 타겟 제한하기
                if (!e.target.matches('.page-item a')) return;

                e.preventDefault(); // a 태그의 링크 이동 기능 중단
        ...
        function makeReplyRemoveClickHandler() {
            ...
            $replyData.onclick = e => {
                ...
                e.preventDefault(); // a 태그의 링크 이동 기능 중지

                // 수정이든 삭제든 댓글 번호가 필요하다.
                // 이벤트가 발생된 곳(수정, 삭제 버튼)에서 가장 가까운 #replyContent에 붙은 댓글 번호 가져오기.
                const rno = e.target.closest('#replyContent').dataset.replyid;

                if (e.target.matches('#replyDelBtn')) {
                    // 삭제 로직 진행
                    /*
                        confirm 함수 이용해서 한 번 더 물어봐 주세요.
